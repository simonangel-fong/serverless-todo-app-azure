locals {
  ...
  # Phase 7 -- hosting (Storage static website, served directly -- no CDN; see storage.tf).
  # Storage account names must be globally unique, 3-24 chars, lowercase alphanumeric only
  # (no hyphens) -- same rationale as function_storage_account_name above. Named distinctly
  # ("...web") so it can never collide with the Phase 5 Functions runtime storage account.
